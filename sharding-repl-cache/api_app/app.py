...
async def root():
    ...
    collections = {}
    for helloDoc in collection_names:
        collection = db.get_collection(helloDoc)
        collections[helloDoc] = {
...
@app.get("/{helloDoc}/count")
async def collection_count(helloDoc: str):
    collection = db.get_collection(helloDoc)
...
@cache(expire=60 * 1)
async def list_users(helloDoc: str):
    """
...
async def show_user(helloDoc: str, name: str):
    """
...
async def create_user(helloDoc: str, user: UserModel = Body(...)):
    """
